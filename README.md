...
│                                                                         │
│  1. 设备发现阶段（红外广播）                                              │
│     基站 ──IR DISCOVER──→ 设备#0,#1,#2,#3                               │
│     基站 ←──IR ANNOUNCE── 设备#0,#1,#2,#3 (各设备上报ID)                  │
│                                                                         │
│  2. 目标选择（基站决定与哪个设备通讯）                                     │
│     基站内部逻辑：根据业务需求选择目标设备（如 0xD0）                        │
│                                                                         │
│  3. 点对点通讯（每次只与一个设备交互）                                     │
│                                                                         │
│     方式1: 红外透传 CAN                                                   │
│     基站 ──IR CAN_REQ──→ 设备#0 (目标)                                   │
│     基站 ←──IR CAN_RSP── 设备#0                                         │
│     (50ms后切换到下一个设备)                                              │
│                                                                         │
│     方式2: ESP-NOW 透传 CAN (扩展功能)                                      │
│     基站 ──IR IDENTIFY──→ 设备#0 (红外确认身份)                           │
│     基站 ←──IR IDENTIFY_RSP── 设备#0                                    │
│     基站 ══ESPNOW PAIR══→ 设备#0 (建立ESP-NOW连接)                       │
│     基站 ══ESPNOW CAN_REQ══→ 设备#0 (透传CAN数据)                       │
│     基站 ══ESPNOW CAN_RSP══← 设备#0                                    │
│                                                                         │
│  4. 轮询调度                                                             │
│     50ms周期: 设备#0 → 设备#1 → 设备#2 → 设备#3 → 设备#0...              │
│     每次只与当前目标设备通讯                                               │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
