...
static void twai_rx_cb(const bsp_twai_msg_t *msg) {
  ...
  static uint32_t s_rx_log_cnt = 0;
  if (++s_rx_log_cnt >= 10) {
    s_rx_log_cnt = 0;
    char log_buf[64];
    snprintf(log_buf, sizeof(log_buf),
             "RX 0x%03X [%02X %02X %02X %02X %02X %02X %02X %02X]",
             (unsigned)msg->id,
...
#if WIRELESSID_ESPNOW_BASE_ENABLE
static void espnow_test_rx_cb(const uint8_t *mac, const uint8_t *data, int len) {
...
#if WIRELESSID_ESPNOW_DEVICE_ENABLE
static void espnow_test_dev_rx_cb(const uint8_t *mac, const uint8_t *data, int len) {
  if (data == NULL || len <= 0) return;
...
#if WIRELESSID_ESPNOW_BASE_ENABLE
  bsp_espnow_register_recv_cb(espnow_test_rx_cb);
...
#if WIRELESSID_ESPNOW_DEVICE_ENABLE
  bsp_espnow_register_recv_cb(espnow_test_dev_rx_cb);
...
#if WIRELESSID_BIZ_ENABLE
  {
    biz_role_t role = WIRELESSID_IS_DEVICE ? BIZ_ROLE_DEVICE : BIZ_ROLE_BASE;
    uint8_t biz_id = WIRELESSID_DEVICE_ID;
